...
void thread_update_priority (struct thread *temp)
{
  ...
  int value = TO_INT_ZERO (INT_SUB (PRI_MAX, aux));
  temp->priority = value > PRI_MAX ? PRI_MAX : value < PRI_MIN ? PRI_MIN : value;
  intr_set_level (old_level);
...
{
  if(!list_empty (&temp->locks_acquired)&&!thread_mlfqs)
  {
