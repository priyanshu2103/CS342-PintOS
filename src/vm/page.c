...
void initialize_supp_page_table (struct hash *supp_page_table)
{
  hash_init (supp_page_table, hash_spte, hash_spte_comp, NULL);
}

struct spt_entry * find_in_spt(struct spt_entry spte)
{
...
static void spt_free (struct spt_entry *spte)
{
  if (spte != NULL)
  {
    if (spte->frame != NULL)
    {
      if((spte->type == FILE && spte->writable) || spte->type == MMAP)
        write_back (spte);
